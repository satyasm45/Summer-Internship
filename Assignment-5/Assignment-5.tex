...
\end{align}
where $\vec{D}$ can be obtained from \eqref{eq:18},\eqref{eq:19} and \eqref{eq:D} and $\vec{c}=\myvec{\beta\\0}$ and range of $\beta$ given by \eqref{eq:23} and ellipse parameters:
\begin{align}
\vec{c} = \myvec{\beta\\0};
